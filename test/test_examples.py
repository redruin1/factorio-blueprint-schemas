...
# def gather_examples(path):
#     return [os.path.join(path, item) for item in os.listdir(path)]

# @pytest.mark.parametrize("filename", gather_examples(os.path.join("test", "examples")))
...
#         grab_json_file("schemas", "2.0.0", "blueprintable.json"),
#         registry=registry_2_0_0
#     )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (AgriculturalTower, "agricultural-tower.json"), 
...
def test_simple_entity(cls, filepath):
    json_dict = cls().to_dict(exclude_defaults=False, exclude_none=False)
